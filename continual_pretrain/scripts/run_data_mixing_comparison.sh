...
# Compares 3 mixing ratios of Romanian CPT (fineweb-ro) vs English Edu (fineweb-edu)
#
# Run 1: 100/0  - pure Romanian (data_builder disabled, single dataset)
# Run 2:  80/20 - 80% Romanian + 20% English edu
# Run 3:  60/40 - 60% Romanian + 40% English edu
#
...
# ── Configuration ─────────────────────────────────────────────────────────────
TOKENS_PER_MILESTONE=1000000   # 25M tokens per milestone
NUM_MILESTONES=1                # 5 milestones = 125M tokens total
LORA_RANK=128
LORA_ALPHA=128
LEARNING_RATE=1e-4
EMBEDDING_LR=2e-5
...
run_experiment() {
    ...
    echo -e "${GREEN}========================================${NC}"
    echo -e "${GREEN}Run ${run_id}/3: ${custom_name}${NC}"
    echo -e "${GREEN}========================================${NC}"
    ...
    echo -e "${GREEN}Run ${run_id}/3 completed!${NC}"
    echo ""
}
...
echo ""
# run_experiment 1 "mix_100_0" \
#     data_builder.enabled=false

# Run 2: 80% Romanian / 20% English edu
echo -e "${BLUE}Run 2: 80% fineweb-ro / 20% fineweb-edu${NC}"
echo -e "${BLUE}  - total_sample_size: ${TOTAL_SAMPLES} (16000 ro / 4000 edu)${NC}"
echo ""
run_experiment 2 "mix_80_20" \
    data_builder.enabled=true \
    data_builder.total_sample_size=${TOTAL_SAMPLES} \
    "data_builder.proportions=[0.8,0.2]"

# Run 3: 60% Romanian / 40% English edu
echo -e "${BLUE}Run 3: 60% fineweb-ro / 40% fineweb-edu${NC}"
echo -e "${BLUE}  - total_sample_size: ${TOTAL_SAMPLES} (12000 ro / 8000 edu)${NC}"
echo ""
run_experiment 3 "mix_60_40" \
    data_builder.enabled=true \
    data_builder.total_sample_size=${TOTAL_SAMPLES} \
    "data_builder.proportions=[0.6,0.4]"
...
echo -e "${YELLOW}  Run 2:  80/20 - Romanian + English edu (light mix)${NC}"
echo -e "${YELLOW}  Run 3:  60/40 - Romanian + English edu (heavier mix)${NC}"
echo -e "${YELLOW}========================================${NC}"
